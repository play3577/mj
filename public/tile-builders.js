...
const TileBuilders = {
  ...
  buildSetTile: (tilenumber, setnum) => li({
    className: "tile",
    dataset: {
      tile: tilenumber,
      setnum
  ...
  buildSet: (set, setnum) => ul(
    set.map(t => TileBuilders.buildSetTile(t, setnum))
  ),

  // build a player.locked set entry, which is
  // a claim object containing a tile array,
  // rather than being a tile array itself.
  buildLockedSet: (claim, setnum) =>
    TileBuilders.buildSet(claim.tiles, setnum),
};
